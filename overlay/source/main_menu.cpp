...
MainMenu::MainMenu()
{
    ...
    this->_masterSlider = new tsl::elm::StepTrackBar("Master", 21);
...
tsl::elm::Element* MainMenu::createUI()
{
    ...
    {
        float newFanLevel = (float)(value * 5) / 100.0f;
        
        for(int i = 0; i < 5; i++) {
            (this->_fanCurveTable + i)->fanLevel_f = newFanLevel;
        }
        
        this->_tableIsChanged = true;
    });
    // Set initial progress based on the first point, or default to 0
